                ...
                {choices.map(choice => (
                    <li key={choice.id}>
                        <button
                            ...
                            className={`w-full text-left px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-sky-500 ${selectedFaces.has(choice.id)
...
interface EagleViewSelectorProps {
    ...
    onSelectionChange: (summary: SelectionSummaryItem[]) => void;
    onFaceSelectionChange?: (faces: Set<string>) => void;
    error: string
    isAllSelected: boolean;
    setIsAllSelected: Dispatch<SetStateAction<boolean>>;
    xmlUISelection: Set<string>;
    setXmlUISelection: Dispatch<SetStateAction<Set<string>>>;
...
const CustomEagleViewSelector: React.FC<EagleViewSelectorProps> = ({
    ...
    const drawScene = useCallback(() => {
        ...
        reportData.facesMap.forEach((face) => {
            ...
            if (faceIndex < 3) { // Log first 3 faces for debugging
            ...
            const isSelected = xmlUISelection.has(face.id);
        ...
        (event: React.MouseEvent<HTMLDivElement>) => {
            ...
            if (isPanningRef.current && wasClick && !disambiguationChoices.length) {
                ...
                if (candidates.length === 1) {
                    ...
                    const newSet = new Set(xmlUISelection);
                    if (newSet.has(clickedFaceId)) {
                        newSet.delete(clickedFaceId);
                    } else {
                        newSet.add(clickedFaceId);
                    }
                    setXmlUISelection(newSet);
                    onSelectionChange(calculateSelectionSummary(newSet, reportData));
                    setIsAllSelected(
                        reportData.facesMap.size > 0 &&
                        newSet.size === reportData.facesMap.size
                    );
                    requestRedraw();
    ...
    const handleSelectAll = useCallback(() => {
        ...
        const isCurrentlyAllSelected = xmlUISelection.size === allFaceIds.size;
        const newSet = isCurrentlyAllSelected ? new Set<string>() : allFaceIds;
    ...
    const handleDisambiguationSelect = (faceId: string) => {
        ...
        onSelectionChange(calculateSelectionSummary(newSet, reportData));
        setIsAllSelected(
            reportData.facesMap.size > 0 && newSet.size === reportData.facesMap.size
        );
        requestRedraw();
        setDisambiguationChoices([]);
    ...
    const handleLegendDoubleClick = useCallback(
        (type: string) => {
            ...
            // Find all faces associated with the given line type
            ...
            if (faceIdsToSelect.size > 0) {
                // Add the new faces to the existing selection
                const newSelectionSet = new Set([
                    ...xmlUISelection,
                    ...faceIdsToSelect,
                ]);

                setXmlUISelection(newSelectionSet);

                onSelectionChange(
                    calculateSelectionSummary(newSelectionSet, reportData)
                );
                setIsAllSelected(
                    reportData.facesMap.size > 0 &&
                    newSelectionSet.size === reportData.facesMap.size
                );
                requestRedraw();
            }
        },
    ...
    return (
        <div className="flex flex-col pt-4 bg-white">
            ...
            <div
                ...
                className="w-full h-[400px] sm:h-[500px] md:h-[600px] border border-gray-300 rounded-md shadow-inner bg-gray-50 relative overflow-hidden"
                ...
                <div className="absolute z-10 top-2 right-2">
                    <div className="flex p-1 space-x-1 bg-white rounded-md shadow">
                        <button
                            ...
                            className={`px-3 py-1 text-sm font-semibold rounded ${viewMode === '2D'
                        ...
                        <button
                            ...
                            className={`px-3 py-1 text-sm font-semibold rounded ${viewMode === '3D'
                                ? 'bg-sky-600 text-white'
                                : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                }`}
                            disabled={!reportData}
                ...
                <div className="absolute z-10 bottom-4 right-4">
                    <div className="flex flex-col p-1 space-y-1 bg-white rounded-md shadow">
                        ...
                        <button
                            ...
                            className={`p-2 rounded-full text-gray-700 hover:bg-gray-200 ${!reportData || reportData.facesMap.size === 0
            ...
            <div className="flex flex-wrap justify-center items-center gap-1.5 mb-3  pt-3 mt-4">
                ...
                {Object.entries(LINE_TYPE_MAP).map(([type, { name, color }]) => {
                    // Calculate total length for this line type
                    const totalLength = reportData?.total2DLengths.get(type) || 0;
                    const isSelected = highlightedLineType === type;
                    
                    // Calculate selected length for this line type
                    let selectedLength = 0;
                    if (reportData && xmlUISelection.size > 0) {
                        reportData.linesMap.forEach(line => {
                            if (line.type === type) {
                                const associatedFaceIds = reportData.lineToFacesMap.get(line.id) || [];
                                const isLineSelected = associatedFaceIds.some(faceId => xmlUISelection.has(faceId));
                                if (isLineSelected) {
                                    const p1 = reportData.pointsMap.get(line.point1Id);
                                    const p2 = reportData.pointsMap.get(line.point2Id);
                                    if (p1 && p2) {
                    ...
                    return (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs rounded-full border-2 transition-all duration-150 ${isSelected
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                {totalLength > 0 && (
                                    <div className="text-xs opacity-75">
                                        ...
                                        {xmlUISelection.size > 0 && selectedLength > 0 && (
                ...
                {/* Total Selected Area in Legend */}
                {xmlUISelection.size > 0 && reportData && (
                    <div className="px-3 py-1.5 text-xs rounded-full border-2 bg-blue-50 border-blue-300">
                        <div className="flex flex-col items-center">
                            ...
                            <span className="text-xs text-blue-600 font-mono">
                                {Array.from(xmlUISelection).reduce((total, faceId) => {
                                    const face = reportData.facesMap.get(faceId);
